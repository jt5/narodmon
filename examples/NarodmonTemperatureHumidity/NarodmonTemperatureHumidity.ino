/*
Скетч для отправки данных температуры и влажности на сервис http://narodmon.ru/
...
#define dataPin  10 // Номер линии, к которой подключен датчик влажности
#define clockPin 11 // Номер линии, к которой подключен датчик влажности
SHT1x sht1x(dataPin, clockPin);
unsigned char RHMacn[8] = {0x00, 'S', 'H', 'T', '1', '1', '_', '1'}; // MAC датчика влажности, должно быть уникальное в пределах системы
...
signed char ReadSensorRH (signed int* pData) {
	// Считываем с датчика влажности
	float RH = sht1x.readHumidity();
